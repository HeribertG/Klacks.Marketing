...
- `FeatureTickerSection.razor` — nahtloses CSS-Marquee: Inhalt liegt zweimal im Markup (Duplikat `aria-hidden`, nur unter `prefers-reduced-motion: no-preference` sichtbar), `translateX(-50%)`-Loop, pausiert bei `:hover`/`:focus-within`, reduced-motion → statische umbrechende Zeile. RTL (ar/he): eigener Keyframe `tickerScrollRtl` (`+50%`), weil die Duplikat-Spur links liegt.
